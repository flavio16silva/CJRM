...
const idade = 47
let precoMessage = null
const isChild = idade <= 7
const isOlder = idade >= 65

if (isChild || isOlder){
  precoMessage = 'Para você, a entrada é grátis!'
} else {
  precoMessage = 'A entrada é R$ 30,00.'
}
console.log(precoMessage)
...
for (let i = 0; i < crazyArray.length; i++){
  const isTypeArray = typeof crazyArray[i]
  const isTypeOfBoolean = isTypeArray === 'boolean'
  const isTypeOfNumber = isTypeArray === 'number'
...
console.log(`O crazyArray tem ${boolean} booleans, ${num} números e ${str} strings.`)
...
for (let i = 0; i < randomNumbers.length; i++){
  const random = randomNumbers[i]
  const isPar = random % 2 == 0
...
const isNumberOdd = impar.join(', ').replace(', 31', ' e 31')
const isNumberEven = par.join(', ').replace(', 58', ' e 58')

console.log(`Numeros ímpares: ${isNumberOdd}. Números pares: ${isNumberEven}.`)
